...
Server.prototype.start = function () {
    ...
    this.io.on('connection', function (socket) {
        ...
        // revebe a informacao de alteracoa de mansagens
...
//                    console.log("\n\n\n\n" +self.hash['.' + parent].modelo.arrayElem[id].conteudo);
        ...
        socket.on('drawClick', function (data) {
            self.hash["." + data.parent].modelo.arrayElem[data.id].canvas = data.canvas;
...
//             I'm sending image back to client just to see and a way of confirmation. You can send whatever.
        ...
        socket.on("getFiles2Folder", function (folder){
            var files = fs.readdirSync(__dirname + '/../public/' + folder).filter(function (file) {
                return file;
            });
            // devolve ao cliente a lista de ficheiros da pasta
            socket.emit("files2folder", files); 
        });
