...
$(document).ready(function () {
	...
	$("body").on('keypress keyup mousedown mouseup click', '.editable', function (e) {
		...
		var tabid = $(this).attr("id").substring(3,$(this).attr("id").length).substring(0,1);

		/*hash = $.map(hash, function(value, index) {
			return [value];
		});*/

		var edit = hash[".txtTab"+tabid].modelo.arrayElem[listClass];

		console.log(edit);

		console.log("hash\n"+hash[".txtTab"+tabid]);

		edit.editor.allOperation(e.handleObj.type, e);
...
function castTab(tabToCast) {
	...
	for (var item in tabToCast.modelo.arrayElem) {
		...
		} else if(tab.modelo.arrayElem[item].elementType === "DIV"){
			console.log(tab);
			tabToCast.modelo.arrayElem[item].editor.socket = socket;
			tab.modelo.arrayElem[item].editor = $.extend(new TextEditor(), tabToCast.modelo.arrayElem[item].editor);
			console.log(tab.modelo.arrayElem[item]);
		}
	}
			...
			for (var elemento in hash[key].modelo.arrayElem) {
				if (hash[key].modelo.arrayElem[elemento].elementType === "IMG") {
					...
					if ($("#" + elemento).attr('class').match('editable')) {
						...
						hash[key].modelo.arrayElem[elemento].createTextEditor(elemento, username, userColor, socket.id, socket);
						hash[key].modelo.arrayElem[elemento].editor(hash[key].modelo.arrayElem[elemento].conteudo);
					}
...
function addtohash(idNum) {
	...
	$(".txtTab" + idNum).children('div').children().each(function () {
		...
		if ($(this).attr("id").match("tab" + tabNumber + "-canvasdr")) {
			...
			allTextEditor.push(editTxt);*/
			tabTest.modelo.arrayElem[thID].createTextEditor($(this).attr("id"), username, userColor, socket.id, socket);
		} else {
			tabTest.modelo.arrayElem[thID] = new Element(thID, thType);
		}
